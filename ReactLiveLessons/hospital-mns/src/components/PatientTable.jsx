...
const statusClass = status =>
    ({
        ...
        "Under Treatment": "text-bg-warning",
...
const PatientTable = ({ refreshKey, setRefreshKey, onEdit, onPatientsChange }) => {
    ...
    useEffect(() => {
        const fetchPatients = async () => {
            ...
            try {
                ...
                setPatients(data);
                onPatientsChange(data);
            } catch {
                setError(
                    "Unable to load patients. Please make sure the API server is running.",
                );
            } finally {
                setIsLoading(false);
            }
    ...
    const handleDelete = async patient => {
        if (!window.confirm(`Remove ${patient.name}'s record? This cannot be undone.`))
            return;
        ...
        try {
            ...
            setError("We couldn't remove this patient. Please try again.");
        } finally {
            setDeletingId(null);
        }
    };
    ...
    return (
        <section className="card border-0 shadow-sm" aria-labelledby="patient-list-title">
            <div className="card-header bg-white border-bottom p-3 p-md-4">
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                        <h2 id="patient-list-title" className="h5 fw-bold mb-1">
                            All patients
                        </h2>
                        <p className="text-secondary small mb-0">
                            {visiblePatients.length}{" "}
                            {visiblePatients.length === 1 ? "record" : "records"} shown
                        </p>
                    </div>
                    <div className="input-group col-12 col-md-5 px-0">
                        ...
                        <input
                            ...
                            placeholder="Search name, doctor, department…"
            ...
            <div className="table-responsive">
                <table className="table table-hover align-middle mb-0">
                    <thead className="table-light">
                        <tr>
                            <th className="ps-3 ps-md-4">Patient</th>
                            <th>Age</th>
                        ...
                        {isLoading ? (
                            ...
                            visiblePatients.map(patient => (
                                <tr key={patient.id}>
                                    ...
                                    <td>
                                        <span
                                            className={`badge rounded-pill ${statusClass(patient.status)}`}>
                                            {patient.status}
                                        </span>
                                    </td>
